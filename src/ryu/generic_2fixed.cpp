...
#include "common.hpp"
#include "digit_table.hpp"
//#include "d2s_intrinsics.hpp"
//#include "d2fixed_full_table.hpp"
...
int g2fixed_buffered_n(long double d, uint32_t precision, char* result) {
  const uint64_t bits = double_to_bits(d);

  // Decode bits into sign, mantissa, and exponent.
  const bool ieeeSign = ((bits >> (DOUBLE_MANTISSA_BITS + DOUBLE_EXPONENT_BITS)) & 1) != 0;
  const uint128_t ieeeMantissa = bits & ((1ull << DOUBLE_MANTISSA_BITS) - 1);
  const uint32_t ieeeExponent = (uint32_t) ((bits >> DOUBLE_MANTISSA_BITS) & ((1u << DOUBLE_EXPONENT_BITS) - 1));
  
  int32_t e2 = (int32_t) ieeeExponent - DOUBLE_BIAS - DOUBLE_MANTISSA_BITS;
  uint128_t m2 = (1ull << DOUBLE_MANTISSA_BITS) | ieeeMantissa;
...
int g2exp_buffered_n(long double d, uint32_t precision, char* result, int* exp_out) {
  const uint64_t bits = double_to_bits(d);
